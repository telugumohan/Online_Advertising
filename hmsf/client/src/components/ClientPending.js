...
const ClientPending = ({name, email}) => {
  const [bookings, setBookings] = useState(null);

  useEffect(() => {
    axios.get('http://localhost:8081/client/booking')
      .then((response) => {
        // Filter the bookings based on name and email
        const filteredBookings = response.data.filter((booking) => booking.name === name && booking.email === email);
